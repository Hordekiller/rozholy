...
function rozholy_redirect_subscribers() {
	...
	if ( ! current_user_can( 'edit_posts' ) && ! current_user_can( 'manage_options' ) ) {
		$dashboard_page = rozholy_get_option( 'dashboard_page', 0 );
...
function rozholy_handle_update_profile() {
	...
	if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'rozholy_update_profile' ) ) {
		wp_die( esc_html__( 'Invalid request.', 'rozholy' ) );
	}
	if ( ! current_user_can( 'edit_posts' ) ) {
		wp_die( esc_html__( 'Unauthorized.', 'rozholy' ), '', 403 );
	}
	...
	if ( isset( $_FILES['avatar'] ) && ! empty( $_FILES['avatar']['name'] ) ) {
		...
		$file_type     = sanitize_mime_type( wp_unslash( $_FILES['avatar']['type'] ) );
...
function rozholy_handle_change_password() {
	...
	if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'rozholy_change_password' ) ) {
		wp_die( esc_html__( 'Invalid request.', 'rozholy' ) );
	}
	if ( ! is_user_logged_in() ) {
		wp_die( esc_html__( 'Unauthorized.', 'rozholy' ), '', 403 );
	...
	$current_pass = isset( $_POST['current_password'] ) ? wp_unslash( $_POST['current_password'] ) : '';
	...
	$login_url = wp_login_url( get_permalink( rozholy_get_option( 'dashboard_page', 0 ) ) );
...
function rozholy_get_avatar( $user_id, $size = 80 ) {
	...
	return get_avatar( $user_id, $size, '', '', array( 'class' => 'dashboard-avatar' ) );
}
